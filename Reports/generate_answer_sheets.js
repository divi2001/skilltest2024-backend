...
const getData = async (center, batchNo, student_id, departmentId) => {
    try {
        ...
        if (student_id) {
            query = "SELECT s.fullname, s.student_id, s.base64, sub.subject_name, d.departmentName, d.departmentExam, d.logo FROM students s JOIN departmentdb d ON s.departmentId = d.departmentId JOIN subjectsdb sub ON s.subjectsId = sub.subjectId AND d.examType = sub.examType WHERE s.center = ? AND s.batchNo = ? AND s.student_id = ? AND s.departmentId = ?;";
            queryParams.push(student_id, departmentId);
        } else {
            query = 'SELECT s.fullname, s.student_id, s.base64, sub.subject_name, d.departmentName, d.departmentExam, d.logo FROM students s JOIN departmentdb d ON s.departmentId = d.departmentId JOIN subjectsdb sub ON s.subjectsId = sub.subjectId AND d.examType = sub.examType WHERE s.center = ? AND s.batchNo = ? AND s.departmentId = ?';
