...
app.use((req, res, next) => {
  res.setHeader(
    ...
    "default-src 'self'; style-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com; script-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com; connect-src 'self' https://www.shorthandonlineexam.in http://3.6.86.1:5000; img-src 'self' data:;"
  );
  next();
});
...
app.use(cors({
  origin: ['*', 'http://3.109.1.101:3000', 'http://3.109.1.101:3001', 'http://3.109.1.101:3002', 'http://43.204.22.53:5000', 'https://www.shorthandonlineexam.in', 'http://3.6.86.1:5000'],
  credentials: true
}));
