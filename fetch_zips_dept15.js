// Download zip files for Department 15 (today's date only)
// from http://13.204.48.33:8000/
...
const BASE_URL = 'http://13.204.48.33:8000/';
const DOWNLOAD_DIR = path.join(__dirname, 'downloaded_zips_dept15');
...
const main = async () => {
    try {
        ...
        console.log(`Fetching zips for Department ${DEPARTMENT_ID}, ALL batches`);
        console.log('Date filter: TODAY only (CURDATE())');
        console.log('='.repeat(70));
        ...
        const [rows] = await connection.query(query, [DEPARTMENT_ID]);

        if (rows.length === 0) {
            console.log(`\n⚠️  No trackrecord entries found for dept ${DEPARTMENT_ID} with today's date.`);
            await connection.end();
            process.exit(0);
        }

        console.log(`\nFound ${rows.length} students with today's trackrecord entries.\n`);
