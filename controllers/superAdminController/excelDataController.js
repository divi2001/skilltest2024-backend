  ...
  try {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      try {
        ...
        const insertQuery = `
          INSERT IGNORE INTO ${tableName} (${validColumns.join(',')}) 
          VALUES ${placeholders}
        `;
...
const processExcelData = async (req, res) => {
  ...
  try {
    ...
    // Enhanced duplicate checking - Log but don't block
    ...
    if (duplicateResult.duplicates.length > 0 || duplicateResult.internalDuplicates.length > 0) {
      console.warn('[WARN] Duplicates found but proceeding with upload:', {
        databaseConflicts: duplicateResult.duplicates.length,
        internalConflicts: duplicateResult.internalDuplicates.length
      });
      // We don't return 400 here anymore, we let the INSERT IGNORE handle it
    }
