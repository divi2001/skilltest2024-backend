...
exports.populateModReviewLog = async (req, res) => {
    ...
    try {
        ...
        SELECT 
            s.student_id,
            s.departmentId, /* Ensure departmentId is selected */
            COALESCE(fps.passageA, tl.texta, 'empty') AS passageA,
            COALESCE(fps.passageB, tl.textb, 'empty') AS passageB,
            s.subjectsId,
            s.qset,
        ...
        for (const row of results) {
            ...
            if (existingRows.length > 0) {
                ...
                `, [
                    ...
                    QPA,
                    QPB,
                    row.departmentId, // Added departmentId
                    row.student_id
                ]);
                updated++;
            } else {
                ...
                await connection.execute(`
                    INSERT INTO modreviewlog 
                    (student_id, passageA, passageB, passageA_word_count, passageB_word_count, 
                    ansPassageA, ansPassageB, subjectId, qset, QPA, QPB, departmentId)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                `, [
                    row.student_id,
                    row.passageA,
        ...
        console.error('Error populating modreviewlog:', error);
        res.status(500).json({ "message": "Internal Server error" });
    }
}