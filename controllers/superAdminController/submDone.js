...
exports.getExpertReviewLogs = async (req, res) => {
    ...
    try {
        ...
        if (qset) {
            query += ' AND qset = ?';
            queryParams.push(parseInt(qset));
        }
        if (expertId) {
            query += ' AND expertId = ?';
        ...
        if (subm_done !== undefined && subm_done !== '') {
            query += ' AND subm_done = ?';
            queryParams.push(parseInt(subm_done));
        }
        ...
        if (subjectId) {
            ...
            const existingQsetNumbers = existingQsets.map(q => parseInt(q.qset));
        ...
        res.json({
            message: results.length > 0 ? "Expert review logs retrieved successfully" : "No expert review logs found with the specified criteria",
...
exports.getModReviewLogs = async (req, res) => {
    ...
    try {
        ...
        if (hold !== undefined && hold !== '') {
            query += ' AND hold = ?';
            queryParams.push(parseInt(hold));
        }
        ...
        res.json({
            message: results.length > 0 ? "Moderator review logs retrieved successfully" : "No moderator review logs found with the specified criteria",
            data: formattedResults,
            count: formattedResults.length
        });
...
exports.resetExpertReviewLogs = async (req, res) => {
    ...
    try {
        ...
        if (resetType === 'subject' && subjectId) {
            ...
            queryParams.push(subjectId, parseInt(qset));
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                qsets: qsets.map(q => ({ subjectId: q.subjectId, qset: parseInt(q.qset) }))
            }
        });
