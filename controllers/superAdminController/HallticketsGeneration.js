...
function formatDate(dateInput) {
  ...
  return isNaN(date) ? dateInput : 
    date.toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' });
}

// Calculate reporting time if not provided
function calculateReportingTime(startTime) {
  if (!startTime) return 'N/A';
  const [hours, minutes] = startTime.split(':').map(Number);
  const reportingTime = new Date();
  reportingTime.setHours(hours - 1, minutes);
  return reportingTime.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' });
}
...
async function generateStudentHallTicket(doc, studentData, assets) {
  ...
  doc
    .fontSize(8)
    .text("Affix", 475, 90, { align: "center" })
    .text("Passport size", 470, 105, { align: "center" })
    .text("colour", 475, 120, { align: "center" })
    .text("Photograph", 470, 135, { align: "center" });
...
async function loadAssets() {
  ...
  return {
    logoFile: fs.existsSync(logoPath) ? logoPath : null,
    qrFile: fs.existsSync(qrPath) ? qrPath : null,
    signFile: fs.existsSync(signPath) ? signPath : null,
    regularFontPath: path.join(fontsDir, 'NotoSansDevanagari-Regular.ttf'),
    boldFontPath: path.join(fontsDir, 'NotoSansDevanagari-Bold.ttf')
  };
}
...
async function loadStudentData() {
  ...
  const requiredFields = ['student_id', 'fullname', 'instituteId', 'subjectsId', 'batchNo', 'batchdate'];
  ...
  return excelData.map(row => ({
    ...
    centerNo: row['center']?.toString(),
    handicap: row['disability'] || " ",
    subjectCode: row['subjectsId']?.toString(),
    batch: row['batchNo']?.toString(),
    date: formatDate(row['batchdate']),
    examTime: row['start_time'],
    password: row['password']?.toString(),
    instituteCode: row['instituteId']?.toString(),
    subject: row['subject_name'],
    reportingTime: row['reporting_time'] || calculateReportingTime(row['start_time']),
    examCenter: row['center_name'] || 'Exam center not specified',
    centerAddress: row['center_address'] || 'Address not specified',
    image: row['base64'] || null
  }));
...
async function downloadHallTicketForStudent(req, res) {
  try {
    ...
    const student = students.find(s => s.seatNo === seatNo);
    if (!student) {
      return res.status(404).json({ error: `No student found with seat number: ${seatNo}` });
    }
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=hallticket_${seatNo}.pdf`);
...
// Download hall tickets for an institute
async function downloadHallTicketsForInstitute(req, res) {
  try {
    const { instituteId } = req.params;
    const students = await loadStudentData();
    ...
    const instituteStudents = students.filter(s => s.instituteId === instituteId);
    if (instituteStudents.length === 0) {
      return res.status(404).json({ error: `No students found for institute ID: ${instituteId}` });
    }
    ...
    const doc = new PDFDocument({
      ...
      margins: { top: 20, bottom: 20, left: 30, right: 30 },
      info: {
        Title: `Hall Tickets for Institute ${instituteId}`,
        Author: 'Examination System'
      },
      autoFirstPage: false
    });
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=halltickets_institute_${instituteId}.pdf`);
    ...
    console.error('Error generating institute hall tickets:', error);
    if (!res.headersSent) {
      res.status(500).json({ error: error.message });
    }
  }
...
// Download all hall tickets as ZIP
async function downloadAllHallTickets(req, res) {
  try {
    const students = await loadStudentData();
    const assets = await loadAssets();
    ...
    // Generate PDF for each institute
    for (const [instituteId, instituteStudents] of Object.entries(instituteGroups)) {
      const pdfPath = path.join(tempDir, `${instituteId}.pdf`);
      const pdfDoc = new PDFDocument({
        size: "A4",
      ...
      for (const student of instituteStudents) {
        await generateStudentHallTicket(pdfDoc, student, {
          ...assets,
          regularFont,
          boldFont
        });
    ...
    console.error('Error generating all hall tickets:', error);
    if (!res.headersSent) {
      res.status(500).json({ error: error.message });
    }
  }
